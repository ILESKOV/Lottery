...
describe("participation logic", function () {
  ...
  it("should revert if lottery is 'CLOSED", async () => {
    await LOT.transfer(player1.address, 500);
    await LOT.connect(player1).approve(Lottery.address, 50);
    await expect(Lottery.connect(player1).participate()).to.be.revertedWith(
  ...
  it("should revert if lottery is in a state of 'CALCULATING_WINNER'", async () => {
    await Lottery.startLottery();
    await LOT.transfer(player1.address, 500);
    await LOT.connect(player1).approve(Lottery.address, 500);
    await Lottery.connect(player1).participate();
  ...
  it("should revert if allowance of player tokens for lottery contract is not enough", async () => {
    await Lottery.startLottery();
    await LOT.transfer(player1.address, 500);
    await LOT.connect(player1).approve(Lottery.address, 49);
    await expect(Lottery.connect(player1).participate()).to.be.revertedWith(
  ...
  it("should success if allowance of player tokens for lottery contract is equal to ticket price", async () => {
    ...
    await LOT.connect(player1).approve(Lottery.address, 50);
    expect(
      await LOT.connect(player1).allowance(player1.address, Lottery.address)
    ).to.equal(50);
    await Lottery.connect(player1).participate();
    expect(
      await LOT.connect(player1).allowance(player1.address, Lottery.address)
    ).to.equal(0);
    expect(await LOT.connect(player1).balanceOf(player1.address)).to.equal(450);
  });

  it("should success if allowance of player tokens for lottery contract is higher than ticket price", async () => {
    await Lottery.startLottery();
    await LOT.transfer(player1.address, 500);
    await LOT.connect(player1).approve(Lottery.address, 100);
    expect(
      await LOT.connect(player1).allowance(player1.address, Lottery.address)
    ).to.equal(100);
    await Lottery.connect(player1).participate();
  ...
  it("should allow to buy multiple lottery tickets per one lottery", async () => {
    ...
    expect(await LOT.connect(player1).balanceOf(player1.address)).to.equal(400);
  });
  ...
  it("should check lottery balance is increased after the appearance of new participant", async () => {
    ...
    await LOT.connect(player2).approve(Lottery.address, 50);
    await Lottery.connect(player1).participate();
    expect(await Lottery.getLotteryBalance()).to.equal(50);
    await Lottery.connect(player2).participate();
    expect(await Lottery.getLotteryBalance()).to.equal(100);
  });
